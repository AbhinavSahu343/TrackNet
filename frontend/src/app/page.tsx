...
export default function Home() { const { isConnected, lastMessage } = useLiveTelemetry(); 
const { processQueue } = useOfflineQueue(); const [trainCount, setTrainCount] = useState(0);
useEffect(() => { if (lastMessage) { setTrainCount((prev) => prev + 1); } }, [lastMessage]);
